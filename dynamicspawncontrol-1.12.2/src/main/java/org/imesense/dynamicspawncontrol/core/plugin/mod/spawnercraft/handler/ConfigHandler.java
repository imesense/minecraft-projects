...
public class ConfigHandler {
    ...
    private static void loadConfig() {
        ...
        mobEssenceToggleList = Arrays.asList(config.get("general", "Toggle Essence Validity", DEFAULT_DISABLED_MOBS, "Mobs which should not have essence (blacklist mode)\nMobs which should have essence (whitelist mode)").getStringList());
        isListBlacklist = config.get("general", "Essence Blacklist Mode", true, "Toggle Essence should operate in blacklist mode (true) or whitelist mode (false)").getBoolean();
        config.getCategory("Egg Mapping").forEach(key, value -> {
