...
public final class LootBoxInWorldGenerator implements IWorldGenerator
{
                         ...
                         World world, IChunkGenerator iChunkGenerator, IChunkProvider iChunkProvider)
    {
        if (world.provider.getDimension() != 0)
        {
            if (DEBUG)
            {
                LogManager.warn("Skipping non-overworld dimension");
            }
        ...
        if (random.nextDouble() >= OVERALL_CHANCE_PER_CHUNK)
        {
            if (DEBUG)
            {
                LogManager.error("No chest spawned in this chunk (failed overall chance)");
            }
    ...
    private void trySpawnChestForTier(World world, Random random, int chunkX, int chunkZ, int tierIndex)
    {
        for (int attempt = 0; attempt < 5; attempt++)
        {
            ...
            if (y == -1)
            {
                if (DEBUG)
                {
                    LogManager.error("No suitable Y found for " + TIERS[tierIndex] + " chest");
                }
            ...
            if (DEBUG)
            {
                LogManager.debug(String.format("Trying to spawn %s chest at [%d, %d, %d]",
            ...
            if (DEBUG)
            {
                LogManager.info(String.format("Spawned %s chest at [%d, %d, %d]",
                        TIERS[tierIndex], x, y, z));
            }
            ...
            if (chest != null)
            {
                if (DEBUG)
                {
                    LogManager.debug("Adding loot to chest...");
                }
    ...
    private boolean isValidSpawnLocation(World world, int x, int y, int z)
    {
        ...
        if (world.isAirBlock(belowPos) || world.getBlockState(belowPos).getMaterial().isLiquid())
        {
            if (DEBUG)
            {
                LogManager.error("Invalid block below at [" + x + ", " + (y - 1) + ", " + z + "]");
            }
        ...
        if (!world.isAirBlock(new BlockPos(x, y, z)))
        {
            if (DEBUG)
            {
                LogManager.error("Target block is not air at [" + x + ", " + y + ", " + z + "]");
            }
        ...
        for (int dx = 0; dx <= 1; dx++)
        {
            for (int dz = 0; dz <= 1; dz++)
            {
                ...
                if (!world.isAirBlock(pos) && !world.getBlockState(pos).getBlock().isReplaceable(world, pos))
                {
                    if (DEBUG)
                    {
                        LogManager.error("Block at [" + pos.getX() + ", " + pos.getY() + ", " + pos.getZ() +
    ...
    private void addLootToChest(TileEntityChest tileEntityChest, Random random, String tier)
    {
        ...
        if (lootTable == null || lootTable.isEmpty())
        {
            if (DEBUG)
            {
                LogManager.warn("Loot table is null or empty for tier: " + tier);
            }
        ...
        if (entries == null || entries.isEmpty())
        {
            if (DEBUG)
            {
                LogManager.warn("No entries found in loot table for tier: " + tier);
            }
        ...
        if (DEBUG)
        {
            LogManager.debug("Adding " + entries.size() + " possible items to chest");
        }
        ...
        for (LootBox.Data entry : entries)
        {
            if (random.nextFloat() > entry.getChance())
            {
                if (DEBUG)
                {
                    LogManager.debug("Item " + entry.getItem() + " failed chance check");
                }
            ...
            if (item == null)
            {
                if (DEBUG)
                {
                    LogManager.warn("Item not found: " + entry.getItem());
                }
            ...
            if (DEBUG)
            {
                LogManager.debug("Added " + count + "x " + entry.getItem() + " to slot " + slot);
            }
        }

        if (DEBUG)
        {
            LogManager.error("Added " + itemsAdded + " items to chest of tier " + tier);
        }
    }
}