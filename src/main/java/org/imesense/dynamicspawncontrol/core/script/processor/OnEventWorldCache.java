...
public final class OnEventWorldCache
{
    ...
    private void checkDayChange(World world)
    {
        ...
        if (newDay != currentDay)
        {
            ...
            if (debugGameEvents)
            {
                LogManager.info(String.format("Day changed: %d -> %d (WorldTime: %d)",
    ...
    private void activateGameEvent(CacheGameEventStorage.GameEventData eventData, int day, WorldServer world)
    {
        if (debugGameEvents)
        {
            ...
            LogManager.info(String.format("Game Event ACTIVATED - Type: %s, Day: %d, Node: %s, Entity: %s, " +
    ...
    public void handleEntitySpawnEvent(LivingSpawnEvent.CheckSpawn event)
    {
        ...
        if (entityData.isContinue != null && entityData.isContinue)
        {
            ...
            LogManager.info(String.format("Entity %s in dimension %d processed with continue: true, result: %s",
    ...
    private Optional<CacheEntityStorage.EntityData> checkActiveNodes(Entity entity, int dimension, ResourceLocation entityKey)
    {
        for (CacheEntityStorage.EntityData entityData : CacheEntityStorage.getInstance().entityData)
        {
            ...
            {
                LogManager.info(String.format("Using active node %d for entity %s",
                                    ...
                                    int dimension, ResourceLocation entityKey)
    {
        ...
        for (CacheGameEventStorage.GameEventData eventData : GAME_EVENT_STORAGE.eventData)
        {
            ...
            if (dayCondition)
            {
                ...
                {
                    String eventType = eventData.repeat ? "Repeating" : "One-time";
                    LogManager.info(String.format("Game Event ACTIVE for spawn - Type: %s, Day: %d, Entity: %s, " +
                                    "Checking limit: %d/%d",
                    ...
                    if (currentCount == 0)
                    {
                        LogManager.info(String.format("DEBUG: No %s entities found in world. Total entities: %d",
                        ...
                        for (Object obj : world.loadedEntityList)
                        {
                            if (obj instanceof Entity)
                            {
                                ...
                                LogManager.info(String.format("DEBUG: Entity: %s, Key: %s",
                ...
                if (currentCount >= eventData.max_entity_count)
                {
                    ...
                    if (debugGameEvents)
                    {
                        LogManager.info(String.format("Game Event BLOCKED spawn - Entity: %s, Count: %d/%d, Result: %s",
